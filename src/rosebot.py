...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_encoder(self, inches, speed):
        ...
        inches_per_degree = WheelCircumference / 360
    ...
    def go_straight_until_intensity_is_less_than(self, intensity, speed):
        ...
        self.go(speed, speed)
        while True:
            sensor = self.sensor_system.color_sensor.get_reflected_light_intensity()
            if sensor < intensity:
                self.stop()
                break
    ...
    def display_camera_data(self):
        """
        Prints the Blob data of the Blob that the camera sees
        (if any).
        """
        blob = self.sensor_system.camera.get_biggest_blob()
        print("Blob X: ", blob.center.x)
        print("Blob Y: ", blob.center.y)
        print("Blob Width: ", blob.width)
        print("Blob Height: ", blob.height)
...
class ArmAndClaw(object):
    ...
    def raise_arm(self):
        ...
        if self.touch_sensor.is_pressed() == 1:
            pass
        while self.touch_sensor.is_pressed() != 1:
    ...
    def calibrate_arm(self):
        ...
        while abs(self.motor.get_position()) < (14.2 * 360):
            self.motor.turn_on(-100)
        self.motor.turn_off()
        self.motor.reset_position()
    ...
    def move_arm_to_position(self, desired_arm_position):
        ...
        """
        while abs(self.motor.get_position()) < desired_arm_position:
            self.motor.turn_on(100)
        self.motor.turn_off()
...
class InfraredProximitySensor(object):
    ...
    def get_distance_in_inches(self):
        ...
        """
        cm_per_inch = 2.54
        distance = 70/cm_per-inch*self.get_distance() / 100
        print(distance)
        return distance
